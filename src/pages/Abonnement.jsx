    ...
    <div style={{ minHeight: '100vh', background: C.bg, fontFamily: 'Lato, sans-serif' }}>
      ...
      <div className="page-content" style={{ maxWidth: 1100, margin: '0 auto', padding: '36px 32px' }}>
        ...
        <div style={{ marginBottom: 28, animation: 'fadeUp 0.5s ease' }}>
          <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, textTransform: 'uppercase', color: C.primary, marginBottom: 8 }}>Facturation</div>
          <h1 className="h1-title" style={{ fontSize: 30, fontWeight: 900, color: C.text, letterSpacing: '-0.8px' }}>Mon abonnement</h1>
        </div>

        {/* Statut actuel */}
        {!loading && abonnement && (
          <div style={{ background: '#fff', borderRadius: 20, padding: '22px 26px', border: `1px solid ${C.border}`, marginBottom: 28, display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, boxShadow: '0 2px 8px rgba(42,22,16,0.04)', animation: 'fadeUp 0.5s ease 0.1s both' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
              ...
              <div>
                ...
                <div style={{ fontSize: 13, color: C.gray, marginTop: 3 }}>
                  {abonnement.plan === 'trial' ? `Essai gratuit jusqu'au ${abonnement.expire_le ? new Date(abonnement.expire_le).toLocaleDateString('fr-FR') : '—'}` : abonnement.details?.periode_fin ? `Renouvellement le ${abonnement.details.periode_fin}` : 'Abonnement actif'}
                </div>
              </div>
            </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 9, marginBottom: 24 }}>
                  {plan.features.map((f, j) => (
                    <div key={j} style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
                      <div style={{ width: 18, height: 18, borderRadius: 5, background: plan.populaire ? 'rgba(255,255,255,0.18)' : 'rgba(181,40,28,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: plan.populaire ? '#fff' : C.primary, flexShrink: 0, fontWeight: 900 }}><IconCheck/></div>
