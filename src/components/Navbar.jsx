...
const IconHeart = ({ size = 18 }) => <svg width={size} height={size} fill="white" stroke="white" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z"/></svg>;

const LIENS = [
  { path: '/dashboard', icon: <IconDash />, label: 'Accueil' },
  ...
  return (
    <>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 24 }}>
          <div onClick={() => navigate('/dashboard')} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer' }}>
            ...
            <div>
              ...
              {restaurant.nom && (
                <div style={{ fontSize: 11, fontWeight: 700, color: C.primary, letterSpacing: '0.3px', lineHeight: 1.2, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                  {restaurant.nom}
                </div>
              )}
            </div>
            ...
            {LIENS.map((l) => {
              ...
              return (
                <button key={l.path} onClick={() => navigate(l.path)} style={{
                  ...
                  transition: 'all 0.2s', cursor: 'pointer', fontFamily: 'Lato, sans-serif',
                }}
      ...
      <div className="bottom-nav" style={{
        ...
        padding: '8px 0 12px', boxShadow: '0 -4px 20px rgba(42,22,16,0.08)',
      }}>
        {LIENS.map(l => {
          const actif = location.pathname === l.path;
          return (
            <button key={l.path} onClick={() => navigate(l.path)} style={{
              ...
              color: actif ? C.primary : C.gray,
              fontFamily: 'Lato, sans-serif', padding: '4px 8px', borderRadius: 10,
            }}>
              {l.icon}
